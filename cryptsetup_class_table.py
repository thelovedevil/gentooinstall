...
class Crypt_Table():
    ...
    def main_curses(self, stdscr):
        ...
        sources = test_crypt_options() # Call the utility function
    ...
    def main(self, stdscr, printer: CursedPrinter, sources):
        ...
        self.screen.clear()
        self.crypt_options_digest(stdscr, printer, sources)
    ...
    def crypt_options_digest(self, stdscr, printer: CursedPrinter, sources):
        ...
        table_height = max_y - 2 # Allocate most of the screen for the table, leave 2 for header/footer
        table_width = max_x // 2
        ...
        def draw_screen():
            stdscr.clear()
            printer.display_text(["Cryptsetup Options Table: Use UP/DOWN to navigate, ENTER to select, 'q' to quit."], row=0, col=0)
            
            # The table widget will draw itself at the correct position
