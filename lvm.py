...
def test_process(printer: CursedPrinter): # This function's context is unclear, assuming it needs printer
    printer.display_text(["Running test_process (luksFormat /dev/sda)..."])
    try:
        subprocess.run(['sudo', 'cryptsetup', 'luksFormat', '/dev/sda'], check=True)
        printer.display_text(["luksFormat on /dev/sda successful."])
    except subprocess.CalledProcessError as e:
        logging.error(f"Error in test_process: {e.stderr.strip()}")
        printer.display_text([f"Error in test_process: {e.stderr.strip()}"], color_pair=2)

def test_processes(printer: CursedPrinter): # This function's context is unclear, assuming it needs printer
    printer.display_text(["Running test_processes (luksOpen /dev/sda gentoo)..."])
    try:
        subprocess.run(['sudo', 'cryptsetup', 'luksOpen', '/dev/sda', 'gentoo'], check=True)
        printer.display_text(["luksOpen on /dev/sda successful."])
    except subprocess.CalledProcessError as e:
        logging.error(f"Error in test_processes: {e.stderr.strip()}")
        printer.display_text([f"Error in test_processes: {e.stderr.strip()}"], color_pair=2)
